...
- **Решение:** Использовать единый топик `GiftCertificates.Stream` с обязательными атрибутами `eventType`, `channel`, `origin`, `correlationId`, позволяющий сегментировать события на стороне потребителей.
- **Последствия:** Все интеграции подписываются на `GiftCertificates.Stream` и фильтруют сообщения по типу; ACL и мониторинг обновлены с учётом консолидации.

...
- **Вопрос:** Где CRM получает данные для отправки CVC?  
  **Ответ:** CRM получает событие-триггер и обращается к сервису процессинга по API, чтобы получить CVC.

- **Вопрос:** Какие потоки Kafka остаются для сервиса процессинга?  
  **Ответ:** Все события и команды подаются через единый поток `GiftCertificates.Stream`, который соответствует корпоративным правилам.

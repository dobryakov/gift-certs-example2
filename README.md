...
В процессе проектирования инструмент задаёт вопросы и советует, какой роли из команды заказчика их переадресовать (например, маркетологу, или директору по снабжению).
...
- `start.txt` — основной регламент: последовательность шагов, правила уточнения требований и корпоративные ограничения.
...
- `specs/business-logic.md` — простое описание происходящих в проекте событий и их последствий.
- `specs/architecture.md` — текстовое описание архитектуры и схема взаимодействия сервисов (Mermaid).
...
- `specs/erd.md` — ER-диаграмма с описанием сущностей и связей, а также исходник для визуализации.
- `specs/sequences.md` — сценарии последовательностей (текстовое описание и Mermaid-диаграммы).
- `specs/flow.md` — flowchart-процессы и сопроводительный текст.
...
- `tools/` — вспомогательные скрипты и docker-команды, используемые при генерации артефактов.

## Как использовать

0. Закинуть в ИИ файл `start.txt`
1. Обсудить с ИИ новые требования или уточнения — ответы фиксируются в `specs/clarification.md`.
2. Попросить ИИ обновить спецификации и диаграммы согласно инструкциям `start.txt`.
